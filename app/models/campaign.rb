class Campaign < ApplicationRecord
  ...
  def check_dates
    if Time.now.hour < 7 || Time.now.hour > 21
      errors.add(:base, "Les campagnes ne peuvent pas être lancées entre 21h et 7h")
    end

    if starts_at.hour < 7 || starts_at.hour > 21
      errors.add(:starts_at, "ne peut pas être entre 21h et 7h")
    end
